...
// MARK: - MacGuestExecTests

private final class MacGuestExecTests {

	/// argv 帶私鑰 + 非互動硬化 + `user@ip` + 遠端命令字串（POSIX 硬跳脫保 argv 邊界）。
	@Test
	private func `ssh arguments carry identity hardening and command`() {
		let exec: MacGuestExec = .init(identity: sampleIdentity(), runner: okRunner())
	...
	private func `nonzero exit is data not error`() async throws {
		let exec: MacGuestExec = .init(identity: sampleIdentity(), runner: okRunner(exit: 42, stdout: "out", stderr: "err"))
	...
	private func `zero exit returns decoded output`() async throws {
		let exec: MacGuestExec = .init(identity: sampleIdentity(), runner: okRunner(exit: 0, stdout: "hello\n"))
	...
	private func `ssh 255 maps to transport failure`() async throws {
		...
		} catch let MacGuestExecError.transportFailure(stderr) {
	...
	/// 執行器擲 timedOut → 映成 ``MacGuestExecError/timedOut(_:)``（帶原逾時長度）。
	@Test
	private func `timeout error maps to timed out`() async throws {
		...
		let exec: MacGuestExec = .init(identity: sampleIdentity(), runner: runner)
		await #expect(throws: MacGuestExecError.timedOut(.seconds(3))) {
	...
	private func `launch failure maps to launch failed`() async throws {
		...
		} catch MacGuestExecError.launchFailed {
			// 預期路徑
		} catch {
	...
	private func `standard input and executable pass through`() async throws {
		...
		let exec: MacGuestExec = .init(identity: sampleIdentity(), runner: runner, sshExecutable: "/usr/bin/ssh")
